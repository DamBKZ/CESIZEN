...
public class InformationServiceImpl implements InformationService {
    ...
    if (req.getType() == null) {
        throw new BadRequestException("Le type d'information est obligatoire.");
    }
    ...
    if (req.getSlug() == null || req.getSlug().isBlank()) {
        throw new BadRequestException("Le slug est obligatoire.");
    }
    ...
    switch (req.getType()) {
        case ARTICLE -> {
            if (req.getContent() == null || req.getContent().isBlank()) {
                throw new BadRequestException("Le contenu est obligatoire pour un article.");
            }
        }
        case VIDEO -> {
            if (req.getVideoUrl() == null || req.getVideoUrl().isBlank()) {
                throw new BadRequestException("L'URL vidéo est obligatoire pour une vidéo.");
            }
        }
        case PDF -> {
            if (req.getPdfUrl() == null || req.getPdfUrl().isBlank()) {
                throw new BadRequestException("L'URL PDF est obligatoire pour un PDF.");
            }
        }
    }
}
...
@Override
@Transactional
public InformationResponse create(InformationRequest req, User currentUser) {
    ...
    Information saved = repo.save(info);
...
@Override
@Transactional
public InformationResponse update(UUID id, InformationRequest req, User currentUser) {
    validateInformationRequest(req);

    Information info = repo.findById(id)
            .orElseThrow(() -> new NotFoundException("Information introuvable."));
    ...
    private void checkCanManage(Information info, User currentUser) {
        if (currentUser == null) {
        ...
        if (info.getOwner() == null || info.getOwner().getUserId() == null) {
            throw new AccessDeniedException("Vous ne pouvez pas modifier cette information.");
        }

        if (!info.getOwner().getUserId().equals(currentUser.getUserId())) {
            throw new AccessDeniedException("Vous ne pouvez modifier que vos propres informations.");
        }
    }
}
